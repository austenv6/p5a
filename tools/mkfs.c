...
mkfs(int nblocks, int ninodes, int size) {
  ...
  //sb is a superblock. the definition of a sb is in fs.h
  sb.size = xint(size);
  ...
  memset(buf, 0, sizeof(buf)); //memset of block sized buffer. Zeroes out the buffer
  memmove(buf, &sb, sizeof(sb)); // copies the superblock into the buffer
  wsect(1, buf); //write sector address 1 with the contents of the buffer! (see wsect)
...
main(int argc, char *argv[])
{
  ...
  fsfd = open(argv[1], O_RDWR|O_CREAT|O_TRUNC, 0666); //opens and truncates image file we'll be writing to
  if(fsfd < 0){
    perror(argv[1]);
    exit(1);
  }
  
  mkfs(995, 200, 1024); // calls mkfs which is a routine that makes a really empty file system
  
  // these routines put a root directory inside the file system
  root_dir = opendir(argv[2]);
  
  root_inode = ialloc(T_DIR); // inode for root directory. there should be a data block containing . and ..
...
{
  // seeks in the image with the first arg sec * block size 512 then writes taht
  // given sector out to that location on the disk.
